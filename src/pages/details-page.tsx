...
export default function DetailsPage() {
  ...
  const handleDeleteClick = async () => {
    ...
    try {
      const response = await axios.delete(import.meta.env.VITE_API_PATH + url, {
  ...
  return (
    <div className="flex-grow bg-light dark:bg-dark">
      <div className="relative flex flex-col">
        ...
        <div className="absolute top-12 flex w-full justify-between px-2 text-lg text-slate-50 sm:top-20 sm:px-8 sm:text-xl lg:px-12 lg:text-2xl">
          <img
            alt="Back"
            src={navigateBackWhiteIcon}
            className="active:scale-click h-5 w-10 cursor-pointer"
            onClick={() => navigate(-1)}
          />
          <div className="flex gap-4">
            {userData?.role === 'ADMIN' && (
              <>
                ...
                <button onClick={() => navigate(`/edit-blog/${postId}`, { state: { post } })}><PenIcon /></button>
              </>
            )}
          </div>
        </div>
        <div className="absolute bottom-6 w-full max-w-xl px-4 text-slate-50 sm:bottom-8 sm:max-w-3xl sm:px-8 lg:bottom-12 lg:max-w-5xl lg:px-12">
          ...
          <p className="text-xs font-semibold text-dark-secondary sm:text-sm">{post.authorName}</p>
          <p className="text-xs text-dark-secondary/80 sm:text-sm">{formatPostTime(post.timeOfPost)}</p>
        </div>
      </div>
      ...
      <div className="container mx-auto flex flex-col space-y-2 px-4 py-6 dark:text-white">
        ...
        <div className="block space-y-4 sm:hidden">
          {relatedPostsLoading
            ? Array(3).fill(0).map((_, index) => <PostMobileViewCardSkeleton key={index} />)
            : relatedCategoryPosts.slice(0, 3).map((post) => <PostMobileViewComponent key={post._id} post={post} />)}
        </div>
        <div className="hidden sm:flex sm:flex-wrap sm:p-3">
          {relatedPostsLoading
            ? Array(4).fill(0).map((_, index) => <PostCardSkeleton key={index} />)
            : relatedCategoryPosts.slice(0, 4).map((post) => <PostCard key={post._id} post={post} />)}
        </div>
      </div>
    </div>
  );
